...
class MyEngine(Engine):
    def __init__(self):
        ...
        parser.add_argument("-n", "--nxc_build_file", help="Path to the NXC build file")
        parser.add_argument("-w", "--walltime", help="Grid5000 booking walltime (in hours)")
        parser.add_argument(
        ...
        parser.add_argument(
                    "-i", "--image-store-ssh", help="Remote store location via ssh (e.g. user@remote.store.org)", default=""
                )
    ...
    def run(self):
        ...
        try:
            ...
            hello_remote = Remote(
                my_command, nodes["frontend"], connection_params={"user": "root"}
            )
            hello_remote.run()
            ...
            cat_remote = Remote(
                my_command2, nodes["frontend"], connection_params={"user": "root"}
            )
            cat_remote.run()
